...
void main() {
    ...
    float sdf;
    if (VAR == 0) {
        vec2 b = vec2(u_resolution.x / u_resolution.y * size, size);
        if (u_resolution.x < u_resolution.y) {
            b = vec2(size, u_resolution.y / u_resolution.x * size);
        }
        sdf = sdRoundRect(st, b, roundness);
...
const ShapeBlur: React.FC<ShapeBlurProps> = ({
  ...
  return <div className={className} ref={mountRef} style={{ width: '100%', height: '100%', pointerEvents: 'none' }} />;
};
